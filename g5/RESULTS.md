...
The `--release 11` rebuild produces a **manifest identical to the release-17 build** — same 21
classes, same 86 probe indices, same `schemaHash` per class, same tier-2 selection — so the
fixture's ground truth transfers exactly rather than approximately. That equality is no longer
merely claimed: `run-g5.sh` diffs the two manifests' full probe layouts and aborts the matrix if
they disagree, and the JDK-11 runs assert against their own.

Both manifests are generated by **the real `ax-static`** (`modules/ax-static/target/ax-static.jar`,
run on JDK 17 because it targets release 17 — the manifest is JSON, so the generator's JDK is
independent of the JVM under test). They used to be generated by `io.auxin.agent.manifest.ManifestTool`,
a stand-in that shipped inside the agent jar; that class is deleted and no fallback generator
exists. The change moved the class count from 20 to 21 (`ax-static` inventories the pure interface
`io.auxin.demo.repo.Repository` with `probeCount: 0`, which the stand-in omitted) and it is what
puts tier-2 under test here at all: `ax-static` auto-selects every entry point, so the demo's three
`com.sun.net.httpserver.HttpHandler` implementations and `App.main` now carry rate/error/latency
instrumentation, where the stand-in produced an empty tier-2 allowlist.
